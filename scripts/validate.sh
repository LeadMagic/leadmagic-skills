...
for skill_dir in "$SKILLS_DIR"/*/; do
    ...
    if [ "$line_count" -gt 500 ]; then
        echo -e "${RED}✗${NC} $skill_name (${rule_count} rules) - ${line_count} lines ${RED}(exceeds 500 limit)${NC}"
        ((errors++))
    elif [ "$line_count" -gt 400 ]; then
        echo -e "${YELLOW}⚠${NC} $skill_name (${rule_count} rules) - ${line_count} lines ${YELLOW}(approaching 500 limit)${NC}"
    else
